        ...
        private bool SaveStock()
        {
            if (ValidateForm() == true)
            {
                try
                {
                    ...
                    if (transactQuantity > oldQuantity)
                    {
                        errorMsg += "Transaction quantity cannot be greater than old quantity" + Environment.NewLine;
                        return false;
                    }
                    else
                    {
                        ...
                        if (Convert.ToInt32(taInventoryTransaction.RemoveStock(transactionDetails, dateTime, productId, wareHouseID, cost, null, oldQuantity, transactQuantity, newQuantity, transactionType, comments)) > 0)
                        {
